...
class FrankaDMPCoatFoldEnvCfg(DirectRLEnvCfg):
    ...
    coat_on_hanger = ParticleClothObjectCfg(
        prim_path="/World/envs/env_.*/Cloth",
        init_state=ParticleClothObjectCfg.InitialStateCfg(pos=(0.6, 0.0, 0.5), rot=(0.5, 0.5, 0.5, 0.5)), # rot=(0, 0, -0.70711, 0.70711)),
        spawn=sim_utils.UsdFileCfg(
            usd_path="/workspace/isaaclab/source/isaaclab_tasks/isaaclab_tasks/sim/particle_cloth_hanger.usd",
            scale=(1.0, 1.0, 1.0),
        ),
    )
...
class FrankaDMPCoatFoldEnv(DirectRLEnv):
    ...
    def _get_absolute_pose(self):
        ...
        ee_pose_2_w = self._robot_1.data.body_state_w[
            :, self.robot_entity_cfg['robot_1'].body_ids[0], 0:7
        ]
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # plt.cla()
        # plt.clf()
        # plt.plot(self.target_joints_buffer[:,0,0:4].cpu().numpy()) # just for env_0 and 4 joints
        # plt.gca().set_prop_cycle(None)
        # plt.plot(self.actual_joints_buffer[:,0,0:4].cpu().numpy(), '--') # just for env_0 and 4 joints
    ...
    def create_joints(self, env_ids):
        for idx in env_ids:
            hanger_1_path = Sdf.Path("/World/envs/env_" + str(int(idx)) + "/Cloth/coat_hanger")
