...
./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/train.py --task Bimanual-Residual-Place-Direct-v0 --num_envs 64 --headless --video --video_interval 6000
"""
...
class FrankaClothEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        self.actions[:,2] = self.actions[:,2].clamp(-1.0, 1.0) / 5
        self.actions[:,0:2] = self.actions[:,0:2].clamp(-1.0, 1.0) / 5
        ...
        absolute_pose[:,0:3] = self._demo_traj[
            self.episode_length_buf*self.cfg.decimation] + self.scene.env_origins
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        terminated[is_not_first_step] = torch.bitwise_or(
            ee_distance[is_not_first_step] > 1.0, ee_distance[is_not_first_step] < 0.2)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        imitation_reward_scale = self.cfg.imitation_reward_scale #  / (1 + self.iteration_step/2e3)
        ...
        if len(is_last):
            ...
            imitation_reward[is_last] = 1 / (1 + imitation_reward[is_last]) * imitation_reward_scale
            self._rewards["imitation_reward"].value = imitation_reward
        ...
        self._rewards["height_reward"].value = 1/(0.1 + mean_cloth_height) * self._rewards["height_reward"].scale
        ...
        self._rewards["corner_x_reward"].value = corner_x_vals * self._rewards["corner_x_reward"].scale
        ...
        for reward_key in self._rewards.keys():
            if self._rewards[reward_key].use: # If the reward is enabled
                if 'penalty' in reward_key:
                    total_reward -= self._rewards[reward_key].value / 100
                else:
                    total_reward += self._rewards[reward_key].value / 100
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        self.performed_traj[env_ids] = 0.0
