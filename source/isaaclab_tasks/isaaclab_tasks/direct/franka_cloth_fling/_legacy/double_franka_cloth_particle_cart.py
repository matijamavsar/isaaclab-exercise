...
class FrankaClothEnvCfg(DirectRLEnvCfg):
    ...
    # reward scales
    default_height_reward_scale = 3.0
    default_proximity_scale = 0.1
    default_grasp_reward_scale = 0.1
    default_action_penalty_scale = 1e-4
    default_rot_reward_scale = 1e-3
...
class FrankaClothEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        for i, robot_key in enumerate(self.robots.keys()):  # Loop through all robots dynamically
            ...
            ee_pose_desired_local = self.actions[:,i,0:7].clone()
            self._ik_controller[robot_key].set_command(ee_pose_desired_local)
            ...
            jacobian = self.robots[robot_key].root_physx_view.get_jacobians()[
                :, self.hand_link_idx[robot_key], :, 
                robot_entity_cfg.joint_ids]

            # Get root pose and joint positions
            current_joint_pos = self.robots[robot_key].data.joint_pos
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        if self.cfg.use_dynamic_rewards:
            ...
            self.cfg.rot_reward_scale = self.cfg.default_rot_reward_scale * (
        ...
        for env_idx in range(self.num_envs):
            ...
            corners = cloth_positions[[0, 100, 10100, -1]]
            mean_corner_height = corners[0:2,2].mean()
            height_reward[env_idx] = mean_corner_height * self.cfg.height_reward_scale
            if mean_corner_height > 0.05:
                height_reward[env_idx] *= 10.0
                if mean_corner_height > 0.1:
                    height_reward[env_idx] *= 30.0

            # ✅ 1. Proximity Reward - Encourage approaching the cloth
            for i, robot_key in enumerate(self.robots.keys()):
                ee_pos_l = self.robots[robot_key].data.body_pos_w[env_idx, self.left_finger_link_idx[robot_key]]
                ee_pos_r = self.robots[robot_key].data.body_pos_w[env_idx, self.right_finger_link_idx[robot_key]]
                ee_pos = (ee_pos_l + ee_pos_r) / 2
                ee_corner_distance = torch.norm(corners[i] - ee_pos)
                ...
                if ee_corner_distance < 0.02 and gripper_action < 0.2:
                    grasp_reward[env_idx] += 5.0 * self.cfg.grasp_reward_scale

                # # Alternative way, encourage gripper to close (action → 0) when close to cloth
                # closure_factor = torch.exp(-ee_corner_distance * 8)  # Exponential decay function
                # grasp_reward[env_idx] += (1.0 - gripper_action) * closure_factor * self.cfg.grasp_reward_scale

                # 3. Compute alignment reward
                gripper_quat = self.robots[robot_key].data.body_quat_w[:, self.hand_link_idx[robot_key]]
                gripper_rot = matrix_from_quat(gripper_quat)  # You may need to define this function

                gripper_z_axis = gripper_rot[:, :, 2]  # Shape: (num_envs, 3)

                world_down = torch.tensor([0, 0, -1], device=self.device, dtype=torch.float32)
                alignment_reward = torch.sum(gripper_z_axis * world_down, dim=-1)  # Dot product

                gripper_downward_reward += alignment_reward * self.cfg.rot_reward_scale  # Scaling factor

        # Penalize Cartesian actions
    ...
    def _get_observations(self) -> torch.Tensor:
        ...
        for robot_key in self.robots.keys():  # Loop through all robots dynamically
            ...
            # Add Cartesian pose
            ee_pose_l = robot.data.body_state_w[:, self.left_finger_link_idx[robot_key], 0:7]
            ee_pose_r = robot.data.body_state_w[:, self.right_finger_link_idx[robot_key], 0:7]
            ee_pose_w = (ee_pose_l + ee_pose_r) / 2
