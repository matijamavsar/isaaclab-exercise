...
class FrankaClothEnvCfg(DirectRLEnvCfg):
    ...
    # reward scales
    default_height_reward_scale = 5.0
    default_proximity_scale = 0.01
    default_grasp_reward_scale = 0.1
    default_action_penalty_scale = 1e-4
    default_rot_reward_scale = 0.01
...
class FrankaClothEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        for i, robot_key in enumerate(self.robots.keys()):  # Loop through all robots dynamically
            ...
            self.actions[:,i,-1] = gripper_actions
            ...
            self.robot_dof_targets[robot_key][:,-2:] = gripper_actions.repeat(1,2)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # TEST: let's try to change reward scales each iteration
        ...
        for env_idx in range(self.num_envs):
            ...
            for i, robot_key in enumerate(self.robots.keys()):
                ...
                if min_distance < 0.02 and gripper_action < 0.2:
                    grasp_reward[env_idx] = 10.0 * self.cfg.grasp_reward_scale
                
                ## Alternative way
                # # Encourage gripper to close (action → 0) when close to cloth
                # closure_factor = torch.exp(-min_distance * 8)  # Exponential decay function
                # grasp_reward[env_idx] = (1.0 - gripper_action) * closure_factor * self.cfg.grasp_reward_scale

                # ✅ 3. Height Reward - Keep lifting **stable over time**
                ...
                gripper_quat = self.robots["robot_1"].data.body_quat_w[:, self.hand_link_idx["robot_1"]]
