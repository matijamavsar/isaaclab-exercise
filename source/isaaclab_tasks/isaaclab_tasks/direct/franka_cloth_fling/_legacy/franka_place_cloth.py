...
class FrankaClothEnvCfg(DirectRLEnvCfg):
    ...
    cloth = ParticleClothObjectCfg(
                ...
                init_state=ParticleClothObjectCfg.InitialStateCfg(pos=(0.0, 0.0, 0.1), rot=(1, 0, 0, 0)),
    ...
    default_lift_reward_scale = 100.0
    default_action_penalty_scale = 1e-4
    default_downward_reward_scale = 1e-3
...
class FrankaClothEnv(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        corners = cloth_positions[:, [510, 600, 9600, 9690], :]  # Corners of the cloth (num_envs, 4, 3)
        ...
        # Height reward based on combined proximity and scaling factor
        # height_factor = torch.sigmoid(-combined_proximity*100 + 3)
        height_factor = 1.0
        height_reward = height_factor * mean_corner_height * self.cfg.height_reward_scale
        ...
        for robot_key in self.robots.keys():
            lift_reward[close_and_gripping] += (torch.exp(
                self.robots[robot_key].data.body_state_w[:, self.left_finger_link_idx[robot_key]][:,9]
                [close_and_gripping]) - 1) * self.cfg.lift_reward_scale
                # * mean_corner_height[close_and_gripping] * self.cfg.lift_reward_scale

        # height_reward[robot_1_close & robot_2_close] = mean_corner_height[
        #     robot_1_close & robot_2_close] * self.cfg.height_reward_scale
        # height_reward[mean_corner_height > 0.07] *= 2
        # height_reward[mean_corner_height > 0.12] *= 3
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        if len(self.default_cloth_positions) == 0:
            ...
            ee_pos_l = self.robots['robot_1'].data.body_pos_w[:, self.left_finger_link_idx[robot_key]]
            ee_pos_r = self.robots['robot_1'].data.body_pos_w[:, self.right_finger_link_idx[robot_key]]
            ee_pos_r1 = (ee_pos_l + ee_pos_r) / 2  # Midpoint of the gripper for all environments

            target_cloth_positions = torch.zeros((self.num_envs, 10201, 3), device=self.device)

            cloth_x = ee_pos_r1[:,0]
            cloth_y_min = ee_pos_r1[:,1] - 0.03
            cloth_y_max = cloth_y_min + 0.7
            cloth_z_max = ee_pos_r1[:,2] + 0.03
            cloth_z_min = cloth_z_max - 0.7

            for i in range(self.num_envs):
                target_cloth_positions[i,:,0] = cloth_x[i]
                target_cloth_positions[i,:,1] = torch.linspace(cloth_y_min[i], cloth_y_max[i], 101).repeat(101)
                target_cloth_positions[i,:,2] = torch.linspace(cloth_z_min[i], cloth_z_max[i], 101).repeat_interleave(101)

            self.default_cloth_positions = target_cloth_positions.clone()
        else:
