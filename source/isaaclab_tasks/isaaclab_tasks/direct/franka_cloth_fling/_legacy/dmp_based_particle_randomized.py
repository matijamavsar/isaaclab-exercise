...
./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/train.py --task DMP-Based-Particle-Randomized --num_envs 128 --max_iterations 16000 --headless --enable_cameras
"""
...
class FrankaDMPClothPlaceEnv(DirectRLEnv):
    ...
    def compute_rewards(self):
        ...
        # Action penalty
        rewards["action_penalty"] = torch.sum(self.last_joint_change ** 2, dim=-1) * self._rewards["action_penalty"].scale
    ...
    def _get_observations(self) -> torch.Tensor:
        ...
        print("Action history shape:", act_hist.shape)

        corner_traj_obs = self.corner_traj[:,::48].reshape(self.num_envs, -1)
        print("Corner trajectory observations shape:", corner_traj_obs.shape)
