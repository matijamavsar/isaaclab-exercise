...
class FrankaClothEnvCfg(DirectRLEnvCfg):
    ...
    default_proximity_scale = 0.3
    default_grasp_reward_scale = 0.5
    default_lift_reward_scale = 50.0
    default_imitation_reward_scale = 3.0
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        for i, robot_key in enumerate(self.robots.keys()):
            ...
            if i == 0:
                ...
                robot_1_close = ee_corner_distance < 0.05
                robot_1_gripping = gripper_action < 0.01
                grasp_reward[robot_1_close & robot_1_gripping] += (
                    1/(gripper_action[robot_1_close & robot_1_gripping] + 1e-3) * self.cfg.grasp_reward_scale)
            elif i == 1:
                proximity_robot_2 = dist_reward  # Robot 2 proximity reward
                robot_2_close = ee_corner_distance < 0.05
                robot_2_gripping = gripper_action < 0.01
                grasp_reward[robot_2_close & robot_2_gripping] += (
                    1/(gripper_action[robot_2_close & robot_2_gripping] + 1e-3) * self.cfg.grasp_reward_scale)

            # Grasping reward (penalize for not closing the gripper near the cloth)
            # closure_factor = torch.exp(-ee_corner_distance * 8)
        ...
        for robot_key in self.robots.keys():
            lift_reward[close_and_gripping] += torch.exp(
                ...
                [close_and_gripping]) * self.cfg.lift_reward_scale 
