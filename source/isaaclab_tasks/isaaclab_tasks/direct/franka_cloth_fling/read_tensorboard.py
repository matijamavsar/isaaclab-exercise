...
log_dirs = [
    "/workspace/isaaclab/logs/rsl_rl/franka_cloth_direct/2025-08-04_16-30-01_nonResidual_pos_lemoreCornerReward",
    "/workspace/isaaclab/logs/rsl_rl/franka_cloth_direct/2025-08-06_10-40-08_nonResidual_pos_lemoreCornerReward",
    "/workspace/isaaclab/logs/rsl_rl/franka_cloth_direct/2025-08-04_17-38-52_nonResidual_pos_lemoreCornerReward_disabledInitMotion",
    "/workspace/isaaclab/logs/rsl_rl/franka_cloth_direct/2025-08-04_17-39-57_nonResidual_pos_lemoreCornerReward_sameLengths"
]

labels = [
    "Full method",
    "No initial probing motion",
    "Same cloth lengths"
]
...
for i, tag in enumerate(reward_tags):
    for j, log_dir in enumerate(log_dirs):
        ...
        steps_joined[i].append(steps)
        values_joined[i].append(values)

for i in range(len(reward_tags)):
    steps_joined[i] = np.concatenate((steps_joined[i][0], steps_joined[i][1][1:]))
    values_joined[i] = np.concatenate((values_joined[i][0], values_joined[i][1][1:]))

# Remove the continuing run
log_dirs.pop(1)
...
for i, tag in enumerate(reward_tags):
    ...
    for j, log_dir in enumerate(log_dirs):
        if j==0:
            steps = steps_joined[i]
            values = values_joined[i]
            steps = steps[:15255]
            values = values[:15255]
        else:
            steps, values = load_scalar_from_log(log_dir, tag)
        ...
        ax.plot(rolling_steps, rolling, color=colors[j], label=labels[j])
...
plt.tight_layout()
plt.savefig("/workspace/isaaclab/logs/snapshots/reward_comparison.pdf")
