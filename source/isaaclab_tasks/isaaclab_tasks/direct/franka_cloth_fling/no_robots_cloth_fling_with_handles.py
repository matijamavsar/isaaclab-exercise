...
class FrankaDMPClothPlaceEnv(DirectRLEnv):
    ...
    def set_handles(self, y):
        ...
        self._handle_1.write_root_pose_to_sim(rarm_pose)
        self._handle_2.write_root_pose_to_sim(larm_pose)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # Reset cloth/handles
        zeros_vel = torch.zeros_like(
            (self._cloth_plain.root_physx_view.get_velocities()))

        zero_idx = torch.arange(0, self.num_envs, device=self.device)
        self._cloth_plain.root_physx_view.set_velocities(zeros_vel, indices=zero_idx)
        self._cloth_plain.root_physx_view.set_positions(self.default_states["_cloth_plain"], indices=zero_idx)
