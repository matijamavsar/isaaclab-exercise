...
class FrankaDMPClothPlaceEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: FrankaDMPClothPlaceEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._demo_traj = torch.cat((self._demo_traj[0].unsqueeze(0), 
                                     self._demo_traj),dim=0)
        self._demo_velocities = torch.diff(self._demo_traj, dim=0)
        self._demo_velocities = torch.cat((self._demo_velocities, self._demo_velocities[-1].unsqueeze(0)))
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        ### This is for velocity residual
        # dmp_parameters = dmp_parameters.clamp(-2.5, 2.5)
        # dmp_parameters[:,0] = 0.0
        # dmp_parameters[:,1] = 0.0
        # dmp_parameters[:,2] = 0.0
        # dmp_parameters[:,3] = 0.0

        ### This is for position residual
        dmp_parameters = dmp_parameters.clamp(-0.8, 0.8)
        current_absolute_pose = self._get_absolute_pose()
        current_absolute_pose[:,0:3] -= self.scene.env_origins
        dmp_parameters[:,0] = 0.0
        dmp_parameters[:,1] = 0.0
        dmp_parameters[:,2] = dmp_parameters[:,2].clamp(-0.8, 0.8)
        dmp_parameters[:,3] = dmp_parameters[:,3].clamp(-0.8, 0.8)
    ...
    def _apply_action(self):
        ...
        if not self.probing_done:
            ...
            ### This is for velocity residual
            # target_y = current_absolute_pose[:, [0, 2]] - self.scene.env_origins[:, [0, 2]]
            # target_y = target_y + self._demo_velocities[self.action_steps][:,[0,2]]*5 + y/120

            ### This is for position residual
            target_y = self._demo_traj[self.action_steps][:,[0,2]] + y
            target_y[:,1][target_y[:,1] < 0.15] = 0.15
