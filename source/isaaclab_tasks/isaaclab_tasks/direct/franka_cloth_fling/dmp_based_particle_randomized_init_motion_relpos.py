...
class FrankaDMPClothPlaceEnv(DirectRLEnv):
    ...
    def _get_arm_poses(self):
        ...
        return ee_pose_1_w, ee_pose_2_w
    ...
    def inverse_kinematics(self, y):
        current_absolute_pose = self._get_absolute_pose()
        curr_rarm, curr_larm = self._get_arm_poses()
        ...
        curr_larm_pos, curr_larm_quat = subtract_frame_transforms(
            root_pose_w[:, 0:3], root_pose_w[:, 3:7], 
            curr_larm[:, 0:3], curr_larm[:, 3:7]
        )
        ...
        curr_rarm_pos, curr_rarm_quat = subtract_frame_transforms(
            root_pose_w[:, 0:3], root_pose_w[:, 3:7], 
            curr_rarm[:, 0:3], curr_rarm[:, 3:7]
        )
        ...
        # Make differential
        rarm_pos_b -= curr_rarm_pos
        larm_pos_b -= curr_larm_pos

        rarm_pose_b = torch.cat((rarm_pos_b, torch.zeros((self.num_envs, 3), device=self.device)), dim=-1)
        larm_pose_b = torch.cat((larm_pos_b, torch.zeros((self.num_envs, 3), device=self.device)), dim=-1)

        self._ik_controller['robot_1'].set_command(rarm_pose_b,
            ee_pos=curr_rarm_pos, ee_quat=curr_rarm_quat)
        self._ik_controller['robot_2'].set_command(larm_pose_b,
            ee_pos=curr_larm_pos, ee_quat=curr_larm_quat)
