...
class FrankaDMPClothPlaceEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: FrankaDMPClothPlaceEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.imitation_buf = torch.zeros((H, self.num_envs, 2), device=self.device)
    ...
    def _apply_action(self):
        ...
        if not self.probing_done:
            ...
            self.imitation_buf = self.imitation_buf.roll(-1, dims=0)
            self.imitation_buf[-1] = current_absolute_pose[:, [0, 2]] - self.scene.env_origins[:, [0, 2]]
    ...
    def inverse_kinematics(self, y):
        ...
        for i, robot_key in enumerate(self.robots.keys()):  # Loop through all robots dynamically
            ...
            if self.cfg.write_joint_state:
                ...
                self.clamped_joint_change = self.joint_change.clamp(-5e-2, 5e-2)
    ...
    def compute_rewards(self):
        ...
        if not self.cfg.use_weighted_atan_rewards and not self.cfg.use_weighted_exp_rewards:
            rewards["height_reward"] = (1.0 / (mean_cloth_height)) * self._rewards["height_reward"].scale
        ...
        imitation_error = torch.norm(self.imitation_buf - self._demo_traj[:,[0,2]].unsqueeze(1), dim=-1).mean(0)
        rewards["imitation_reward"] = 1 / (0.1 + imitation_error)
