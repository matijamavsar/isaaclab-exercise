...
class FrankaDMPClothPlaceEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: FrankaDMPClothPlaceEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # self.compiled_get_q_solution = torch.compile(self.get_q_solution)
    ...
    def _apply_action(self):
        ...
        if not self.probing_done:
            ...
            self.inverse_kinematics_curobo(target_y)
            for robot_key in self.robots.keys():
                # ✅ Set joint position target separately for each robot
                if self.cfg.write_joint_state:
                    q_des = self.robot_dof_targets[robot_key][:, :7] 
    ...
    def inverse_kinematics_curobo(self, y):
        ...
        with torch.inference_mode(False):
            for robot_key in self.robots.keys():
                current_joints[robot_key] = self.robots[robot_key].data.joint_pos[
                    :, self.robot_entity_cfg[robot_key].joint_ids].clone()
            q_solution = self.get_q_solution(current_joints, arm_pos_b, arm_quat_b)
            # q_solution = self.compiled_get_q_solution(current_joints, arm_pos_b, arm_quat_b)
        ...
        for i, robot_key in enumerate(self.robots.keys()):
            ...
            joint_change = new_targets - current_joints[robot_key]
            self.action_penalties[robot_key][self.action_steps] = joint_change.abs().mean(dim=-1)

            # max_joint_change = joint_change.abs().max(dim=1, keepdim=True)[0]  # [n_env, 1]
            # scale_factor = self.cfg.joint_vel_max * self.physics_dt / (max_joint_change + 1e-8)  # Add epsilon to avoid divide-by-zero
            # scaled_joint_change = joint_change * scale_factor
            # new_targets = current_joints[robot_key] + scaled_joint_change

            new_targets = current_joints[robot_key] + joint_change
    ...
    def get_q_solution(self, current_joints, arm_pos_b, arm_quat_b):
        q_solution = {}
        for robot_key in self.robots.keys():
            goal = Pose(arm_pos_b[robot_key].clone(), arm_quat_b[robot_key].clone())
            result = self.ik_solver.solve_batch(goal,
                retract_config=current_joints[robot_key],
                seed_config=current_joints[robot_key].unsqueeze(0).repeat(self.num_envs,1,1))
