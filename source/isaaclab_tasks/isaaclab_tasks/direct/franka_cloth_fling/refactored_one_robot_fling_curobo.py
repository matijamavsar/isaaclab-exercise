...
import carb
from pxr import UsdShade, UsdGeom, UsdPhysics, Sdf, Gf, UsdLux
import omni.usd
...
class FrankaDMPClothPlaceEnvCfg(DirectRLEnvCfg):
    ...
    rewards = {
        ...
        "height_reward": Reward(True, 10.0),
        "corner_x_reward": Reward(True, 5.0),
        "direction_reward": Reward(True, 5.0),
...
class FrankaDMPClothPlaceEnv(DirectRLEnv):
    ...
    def _apply_action(self):
        ...
        current_abs = self._get_absolute_pose()
        y_coordinate = (current_abs[:,0:3] - self.scene.env_origins)[:,1]
        self.y_values[self.action_steps] = y_coordinate
    ...
    def get_q_solution(self, current_joints, arm_pos_b, arm_quat_b):
        goal = Pose(arm_pos_b.clone(), arm_quat_b.clone())
    ...
    def inverse_kinematics_single(self, y_xz: torch.Tensor):
        ...
        with torch.inference_mode(False):
            current_joints = self._robot_1.data.joint_pos[
                :, self.robot_entity_cfg["robot_1"].joint_ids].clone()
            q_solution = self.get_q_solution(current_joints, target_pos_b, target_quat_b)

        # Compute new joint positions using IK
        self.robot_dof_targets["robot_1"][:, 0:7] = q_solution

        padded_joint_pos = F.pad(input=current_joints, pad=(0,2), mode='constant', value=0)
