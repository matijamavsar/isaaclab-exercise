...
def _run_one_cloth(args):
    ...
    inds = [0]
    for k in range(tf):
        cloth.simulate(u=u[k], control=inds)
...
class FrankaDMPClothPlaceEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: FrankaDMPClothPlaceEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.y_buffer = torch.zeros((self.cfg.max_episode_length, self.num_envs, 3), device=self.device)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        dmp_parameters[:, 5] = dmp_parameters[:, 5].clamp(0.01, 0.2)
