...
class FrankaDMPClothPlaceEnvCfg(DirectRLEnvCfg):
    ...
    rewards = {
        ...
        "height_reward": Reward(False, 1e-1),
...
class FrankaDMPClothPlaceEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: FrankaDMPClothPlaceEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.n = n
        np.random.seed(1)
        # X, T = createRectangularMesh(a = 0.7, b = 0.7, na = na, nb = nb, h = 0.0)
        X, T = createRectangularMesh(a = 0.7, b = 0.7, na = na, nb = nb, h = 0.33)
        X[:,2] += 0.01; 
        ...
        for i in range(self.num_envs):
            ...
            TT = T.copy()
            self.simcloth_objects.append(XX)
            self.t_objects.append(TT + i*n*n)

        XX = np.concatenate(self.simcloth_objects)
        TT = np.concatenate(self.t_objects)

        self.simcloth = Cloth(XX, TT) 
        self.simcloth.setSimulatorParameters(
    ...
    def _apply_action(self):
        for i in range(self.num_envs):
            cloth_pos = torch.tensor(self.simcloth.positions.reshape(self.num_envs,-1)[i], device=self.device)
            cloth_vel = torch.tensor(self.simcloth.velocities.reshape(self.num_envs,-1)[i], device=self.device)
            self._cloth_objects[i].root_physx_view.set_positions(cloth_pos, 
                                                                 torch.tensor([0], device=self.device))
            self._cloth_objects[i].root_physx_view.set_velocities(cloth_vel, 
                                                                 torch.tensor([0], device=self.device))

        inds_ctr = [(i*self.n**2) for i in range(self.num_envs)]
        self.simcloth.simulate(u=self.simcloth.positions[inds_ctr], control=inds_ctr)
        ...
        cloth_positions = self._get_cloth_positions_shaped()
        side_len = int(torch.sqrt(torch.tensor(cloth_positions.shape[1])).to(self.device))
        midpoints = cloth_positions[:, [side_len//2 - 1, -1, 0, 200], :]
        self.cloth_pos_buffer[self.action_steps[0]] = midpoints
    ...
    def compute_rewards(self):
        ...
        free_x = corners[:, [0, 1], 0] - self.scene.env_origins[:, 0].unsqueeze(1)
        grasped_x = corners[:, [2, 3], 0] - self.scene.env_origins[:, 0].unsqueeze(1)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        if not hasattr(self, "default_states") or self.default_states is None:
            self.default_states = {}
            self.default_states["_cloth_plain_pos"] = self.simcloth.positions.copy()
            self.default_states["_cloth_plain_vel"] = self.simcloth.velocities.copy()

        self.simcloth.positions = self.default_states["_cloth_plain_pos"]
        self.simcloth.velocities = self.default_states["_cloth_plain_vel"]
        self.simcloth.history_pos = []
        self.simcloth.history_vel = []

        plt.clf(); plt.cla()
        for i in range(4):
            if i==0:
                plt.plot(self.cloth_pos_buffer[:,0,i,2].cpu()-0.016, label='Node ' + str(i)) # plot Z of corners for first env
            else:
                plt.plot(self.cloth_pos_buffer[:,0,i,2].cpu(), label='Node ' + str(i)) # plot Z of corners for first env
        plt.legend()
        plt.xlabel('Time step')
        plt.ylabel('Node Z value')
        plt.savefig('./logs/tmp_clothilde.pdf')
