...
from .autoencoder import DepthAutoencoder
...
class FrankaDMPClothPlaceEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: FrankaDMPClothPlaceEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.autoencoder = DepthAutoencoder(device=self.device)
    ...
    def _apply_action(self):
        ...
        if self.action_steps[-1] == self.num_steps_to_settle:
            ...
            dmp_parameters[:, 5] = dmp_parameters[:, 5].clamp(0.1, 0.7)
        ...
        if (self.action_steps[-1] > self.num_steps_to_settle).all():
            ...
            for i in range(self.num_envs):
                current_cloth_pos[i, self.grab_node_idx[i]] = yy[i]
    ...
    def _get_observations(self) -> torch.Tensor:
        ...
        elif not self.cfg.use_autoencoder:
        ...
        ##### OR #####
            ...
            if getattr(self, "_ee_camera", None) is not None:
                ...
                with torch.inference_mode(False):
                    depth_in = depth_norm.clone()
                    img_out, latent_space = self.autoencoder(depth_in.unsqueeze(1))
                    img_loss = F.mse_loss(img_out.squeeze(1), depth_in)
                    self.optimizer.zero_grad()
                    img_loss.backward()
                    self.optimizer.step()

                observations = latent_space.mean(-1).mean(-1)
