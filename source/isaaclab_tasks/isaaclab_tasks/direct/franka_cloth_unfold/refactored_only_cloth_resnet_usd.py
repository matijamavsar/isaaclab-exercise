...
class FrankaDMPClothPlaceEnv(DirectRLEnv):
    ...
    def move_grasp_sphere(self, env_idx, new_world_pos):
        ...
        sphere_prim = f"{env_path}/GraspSphere"
        xform = UsdGeom.Xformable(get_prim_at_path(sphere_prim))

        # try:
        #     translate_op = xform.GetTranslateOp()
        #     translate_op.Set(Gf.Vec3f(float(new_world_pos[0]),
        #                               float(new_world_pos[1]),
        #                               float(new_world_pos[2])))
        # except:
        #     pass

        sphere_prim = get_prim_at_path(sphere_prim)
        physx_rb = PhysxSchema.PhysxRigidBodyAPI.Apply(sphere_prim)
        breakpoint()
        physx_rb.CreateKinematicTargetPositionAttr().Set(
            Gf.Vec3f(float(new_world_pos[0]),
                    float(new_world_pos[1]),
                    float(new_world_pos[2]))
    )
        # sphere_prim.Unload()
    ...
    def _create_grasp_sphere(self, env_idx, world_pos, radius=0.015):
        stage = self.scene.stage
        env_path = f"/World/envs/env_{env_idx}"
        sphere_path = f"{env_path}/GraspSphere"

        # ------------------------------------------------------------------
        # Delete previous grasp sphere if it exists
        # ------------------------------------------------------------------
        if stage.GetPrimAtPath(sphere_path).IsValid():
            omni.kit.commands.execute("DeletePrims", paths=[sphere_path])

        # ------------------------------------------------------------------
        # Create NEW rigid sphere using physicsUtils
        # This automatically sets: collider + rigid body + mass properties
        # ------------------------------------------------------------------
        # sphere_prim = physicsUtils.add_rigid_sphere(
        #     stage=stage,
        #     path=sphere_path,
        #     radius=radius,
        #     position=Gf.Vec3f(float(world_pos[0]), float(world_pos[1]), float(world_pos[2])),
        #     density=1.0,
        # )

        sphere_prim = physicsUtils.add_rigid_box(
            stage, 
            sphere_path, 
            size=Gf.Vec3f(0.01), 
            position=Gf.Vec3f(float(world_pos[0]), float(world_pos[1]), float(world_pos[2])),
        ...
        # ------------------------------------------------------------------
        # Make sphere kinematic (not affected by gravity / forces)
        # ------------------------------------------------------------------
        UsdPhysics.RigidBodyAPI.Apply(sphere_prim)
        rb_api = UsdPhysics.RigidBodyAPI.Apply(sphere_prim)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        if self.try_count[-1] > 0:
            # Create grasp sphere at detected node
            for i in range(self.num_envs):
                self.grab_node_idx, self.cloth_pos = self._get_node_idx_from_start_pos(self.actions)
                node_world_pos = (self.cloth_pos[i, self.grab_node_idx[i]] - self.scene.env_origins[i]).cpu().numpy()
                node_world_gf = Gf.Vec3f(float(node_world_pos[0]), float(node_world_pos[1]), float(node_world_pos[2]))

                sphere_path = self._create_grasp_sphere(env_idx=i, world_pos=node_world_gf, radius=0.15)
                self.grasp_sphere_path = sphere_path

                # Attach cloth node to sphere
    ...
    def _apply_action(self):
        ...
        if self.try_count[-1] > 0 and self.action_steps[-1] == 1:
            for i in range(self.num_envs):
                ...
                att = get_prim_at_path(f"/World/envs/env_{i}/GraspSphere/attachment_0")
