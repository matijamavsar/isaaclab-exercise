...
from isaaclab.assets import ParticleClothObject, ParticleClothObjectCfg, RigidObject, RigidObjectCfg
...
from pxr import Usd, Gf, Sdf, UsdGeom, UsdShade, UsdLux, UsdPhysics, PhysxSchema
...
class FrankaDMPClothPlaceEnvCfg(DirectRLEnvCfg):
    ...
    cuboid = RigidObjectCfg(
        ...
        spawn=sim_utils.CuboidCfg(
            ...
            mass_props=sim_utils.MassPropertiesCfg(mass=5000),
            collision_props=sim_utils.CollisionPropertiesCfg()),
        init_state=RigidObjectCfg.InitialStateCfg(pos=(0.2, 0.0, 0.5))
        )
    ...
    rewards = {
        "spread_reward": Reward(False, 10.0),
        "height_reward": Reward(True, 1.0),
        "target_area_reward": Reward(False, 1.0),
        "grasp_repeat_penalty": Reward(False, 5.0),
    }
...
def soft_clamp(x, low, high, alpha=5.0):
    """
    Smoothly clamp x to [low, high].
    alpha controls how hard the boundary is.
    """
    mid = 0.5 * (low + high)
    half = 0.5 * (high - low)
    return mid + half * torch.tanh(alpha * (x - mid) / (half + 1e-6))
...
class FrankaDMPClothPlaceEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: FrankaDMPClothPlaceEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.try_count = torch.zeros(self.num_envs, device=self.device, dtype=torch.int32)
        self.grasp_translate_op = [[]]
    ...
    def move_grasp_sphere(self, env_idx, new_world_pos):
        new_world_pose = self._cube.data.root_pose_w[env_idx].clone()
        new_world_pose[..., 0:3] = new_world_pos.clone()
        self._cube.write_root_pose_to_sim(
            new_world_pose, env_ids=torch.tensor([env_idx], device=self.device))

    def attach_cloth_particle_to_sphere(self, cloth_prim_path, sphere_prim_path):
        stage = self.scene.stage

        # attachment prim path under the cloth object
        attachment_path = f"{sphere_prim_path}/attachment_0"

        # Create the attachment prim
        attachment = PhysxSchema.PhysxPhysicsAttachment.Define(stage, attachment_path)
        attachment.GetActor0Rel().SetTargets([cloth_prim_path])
        attachment.GetActor1Rel().SetTargets([sphere_prim_path])

        # # Tell PhysX to compute and maintain the attachment constraint
    ...
    def _setup_scene(self):
        ...
        self._cube = self.cfg.cuboid.class_type(self.cfg.cuboid)
        ...
        for env_idx in range(self.scene.num_envs):
            ...
            # cloth_prim.CreateAttribute("primvars:doNotCastShadows", Sdf.ValueTypeNames.Bool).Set(True)
            # cloth_prim.CreateAttribute("primvars:enableShadowTerminatorFix", Sdf.ValueTypeNames.Bool).Set(False)
            # cloth_prim.CreateAttribute("primvars:disableRtSssTransmission", Sdf.ValueTypeNames.Bool).Set(True)
            # cloth_prim.CreateAttribute("primvars:disableAutoLod", Sdf.ValueTypeNames.Bool).Set(True)
    ...
    def _get_node_idx_from_start_pos(self, dmp_parameters, radius_m: float = 0.01):
        """
        For each env, return the index of the cloth node that:
        1) lies within 'radius_m' (default 1 cm) of start_xy in the XY plane, and
        ...
        # [E*N, 3] -> [E, N, 3]
        cloth_positions = self._get_cloth_positions_shaped()
        dmps = dmp_parameters.clone()
        dmps[:,0:3] += self.scene.env_origins
        dmps[:,3:6] += self.scene.env_origins

        start_xy = dmp_parameters[:, 0:2].to(cloth_positions.device)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        if not hasattr(self, 'attachment_created'):
            for i in range(self.num_envs):
                sphere_path = f"/World/envs/env_{i}/cuboid"
                self.attach_cloth_particle_to_sphere(
                    cloth_prim_path=f"/World/envs/env_{i}/Cloth",
                    sphere_prim_path=sphere_path,
                )
            self.attachment_created = True

        if self.try_count[-1] > 0:
            # Get grab nodes
            self.grab_node_idx, self.cloth_pos = self._get_node_idx_from_start_pos(self.actions)
            print("Grab idx:", self.grab_node_idx)

            # current grasp XY in env-local frame
            current_xy = self.actions[:, 0:2].detach()

            if self.prev_grasp_xy is None:
                self.prev_grasp_xy = current_xy.clone()
            else:
                self.delta_grasp_xy = torch.norm(
                    current_xy - self.prev_grasp_xy, dim=-1
                )

            # update memory for next try
            self.prev_grasp_xy = current_xy.clone()
    ...
    def _apply_action(self):
        ...
        if self.try_count[-1] > 0 and self.action_steps[-1] == 0:
            ...
            # dmp_parameters[:, 5] = (dmp_parameters[:, 5]).clamp(0.0, 0.3)

            ### Set cloth node start and goal
            dmp_parameters[:, 0:2] = soft_clamp(
                dmp_parameters[:, 0:2], -clamp_loc, clamp_loc, alpha=6.0)

            dmp_parameters[:, 3:5] = soft_clamp(
                dmp_parameters[:, 3:5], -clamp_loc, clamp_loc, alpha=6.0)
            dmp_parameters[:, 5] = soft_clamp(
                dmp_parameters[:, 5], 0.0, 0.3, alpha=8.0)
            ...
            for i in range(self.num_envs):
                dmp_parameters[i, 0] = self.cloth_pos[i, self.grab_node_idx[i], 0]
                dmp_parameters[i, 1] = self.cloth_pos[i, self.grab_node_idx[i], 1]
                dmp_parameters[i, 2] = self.cloth_pos[i, self.grab_node_idx[i], 2]
        ...
        if self.try_count[-1] > 0 and self.action_steps[-1] > 0 and self.action_steps[-1] < (self.cfg.max_episode_length - self.num_steps_to_settle):
            t, y, dy, ddy = self.dmp_integrator.step()
            self.dmp_integrator.x[t >= self.actions[:, -1]] = 0.1353
            yy = y.clone()
            yy += self.scene.env_origins

            # if not (self.action_steps[-1] == self.num_steps_to_settle + 1).all():
            #     max_speed = 0.2  # m/s
            #     dt = getattr(self, "physics_dt", 1.0/120.0)
            #     max_step = max_speed * dt

            #     delta = yy - self.prev_y                           # (B, 3)
            ...
            #     yy = self.prev_y + delta * scale

            # self.prev_y = yy

            # Clamp y to acceptable ranges and do not clamp DMP weights
            yy -= self.scene.env_origins
            yy[:,0] = yy[:,0].clamp(-clamp_loc, clamp_loc)
            yy[:,1] = yy[:,1].clamp(-clamp_loc, clamp_loc)
            yy[:,2] = yy[:,2].clamp(0.0, 0.3)
            yy += self.scene.env_origins

            for i in range(self.num_envs):
                new_pos = yy[i]
                self.move_grasp_sphere(i, new_pos)
        ...
        # Set attachments at beginning of episode
        if self.try_count[-1] > 0 and self.action_steps[-1] == 1:
            for i in range(self.num_envs):
                node_world_pos = self.cloth_pos[i, self.grab_node_idx[i]].cpu().numpy()
                node_world_gf = Gf.Vec3f(float(node_world_pos[0]), float(node_world_pos[1]), float(node_world_pos[2]))
                cloth_prim_path = f"/World/envs/env_{i}/Cloth"
                cloth_prim = get_prim_at_path(cloth_prim_path)
                cloth_xf = UsdGeom.Xformable(cloth_prim).ComputeLocalToWorldTransform(0.0)
                cloth_local = cloth_xf.GetInverse().Transform(node_world_gf)

                att = get_prim_at_path(f"/World/envs/env_{i}/cuboid/attachment_0")
                att.GetAttribute('points0').Set(
                    [cloth_prim.GetAttribute('points').Get()[int(self.grab_node_idx[i])]])
                att.GetAttribute('points1').Set([Gf.Vec3f(0.0, 0.0, 0.0)])
                att.SetActive(False)   # release
                att.SetActive(True)    # grasp

        if self.try_count[-1] > 0 and self.action_steps[-1] == (self.cfg.max_episode_length - self.num_steps_to_settle):
            for i in range(self.num_envs):
                att = get_prim_at_path(f"/World/envs/env_{i}/cuboid/attachment_0")
                att.GetAttribute('points0').Set([])
                att.GetAttribute('points1').Set([])
                att.SetActive(False)   # release
                att.SetActive(True)    # grasp
                rest_pos = torch.tensor([0.2, 0.0, 0.2], device=self.device) + self.scene.env_origins[i]
                self.move_grasp_sphere(i, rest_pos)

        self.iteration_step += 1
        self.action_steps += 1

    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        # A try finishes when the episode time exceeds the DMP duration + settling
        # Here we use your episode_length_buf to detect completion of a single try.
        done_this_step = self.episode_length_buf >= self.max_episode_length

        # Mark those envs as truncated=done
        truncated = done_this_step.clone()

        # terminated is always zero in your environment
    ...
    def compute_rewards(self):
        ...
        if hasattr(self, "delta_grasp_xy"):
            beta = 1.0  # controls how fast penalty decays
            penalty = torch.exp(-beta * self.delta_grasp_xy)
        else:
        ...
        rewards["grasp_repeat_penalty"] = penalty * self._rewards["grasp_repeat_penalty"].scale
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # ----------------------------------------------------
        # 🔒 Disable learning during settling try (try_count == 0)
        # ----------------------------------------------------
        settling_mask = self.try_count == 0
        total_reward[settling_mask] = 0.0

        # grasp_repeat_penalty = self.same_grasp.float() * 0.5
        # total_reward -= grasp_repeat_penalty

        if self.try_count[-1] == self.cfg.num_tries:
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        needs_full_reset = self.try_count[env_ids] > self.cfg.num_tries

        if needs_full_reset.any():
            # For envs that completed all tries: reset cloth
            envs_to_reset = env_ids[needs_full_reset]

            zeros_vel = torch.zeros(
                (len(envs_to_reset), self._cloth_objects[0].root_physx_view.max_particles_per_cloth * 3),
                device=self.device)
            zero_idx = torch.tensor([0], device=self.device)

            for e_idx in envs_to_reset:
                self._cloth_objects[e_idx].root_physx_view.set_velocities(zeros_vel[e_idx], indices=zero_idx)
                self._cloth_objects[e_idx].root_physx_view.set_positions(
                    self.default_states["_cloth_plain"][e_idx], indices=zero_idx)
                self._cloth_objects[e_idx].update(self.physics_dt)

            # Reset try counter for these envs
            self.try_count[envs_to_reset] = 0

            self.prev_grasp_xy = None

            # for i in range(self.num_envs):
            #     env_path = f"/World/envs/env_{i}"
            #     sphere_path = f"{env_path}/GraspSphere"
            #     att_path = f"{env_path}/GraspSphere/attachment_0"

            #     if self.scene.stage.GetPrimAtPath(sphere_path).IsValid():
            #         omni.kit.commands.execute("DeletePrims", paths=[sphere_path, att_path])
    ...
    def _get_observations(self) -> torch.Tensor:
        ...
        ##### OR #####
            ...
            if getattr(self, "_ee_camera", None) is not None:
                ...
                observations = depth_norm.unsqueeze(1).repeat(1,3,1,1)
                ...
                # img = depth_norm[0].detach().cpu()
                # plt.imsave(
                #     f"/workspace/isaaclab/source/isaaclab_tasks/isaaclab_tasks/depth.jpg",
                #     img,
                #     cmap="viridis",
                #     vmin=0.0,
                #     vmax=1.0,
                ...
                # return {
                #     "policy": observations,          # 2D → CNN path (B,C,H,W)
                #     "critic": observations,
                # }

                dummy = torch.zeros((self.num_envs, 1), device=self.device)
                return {
                    "policy": {},
                    "critic": {},
                    "state": dummy,
