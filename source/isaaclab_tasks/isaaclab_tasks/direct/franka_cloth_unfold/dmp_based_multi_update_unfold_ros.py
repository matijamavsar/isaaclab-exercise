...
class FrankaDMPClothPlaceEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: FrankaDMPClothPlaceEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        for robot_key, robot in self.robots.items():
            ...
            for env_i in range(self.num_envs):
                ...
                cmd_topic   = f"{base_ns}/command_joint_state"
                ...
                def _make_cb(rk, ei):
                    def _cb(msg: JointState):
                        # map by names if provided, else by index
                        dof_local = self.robots[rk].num_joints
                        q = torch.zeros(dof_local, device=self.device)
                        ...
                        if msg.name and len(msg.name) == len(msg.position):
                            ...
                            for j, n in enumerate(self._ros_joint_names[rk][:dof_local]):
                                ...
                                if src is not None:
                                    q[j] = float(msg.position[src])
                        else:
                            # fallback by index
                            if len(msg.position) >= dof_local:
                                q = torch.tensor(msg.position[:dof_local], device=self.device)
                            else:
                                return  # ill-formed, ignore

                        self._ros_desired_q[rk][ei] = q
                        self._ros_desired_t[rk][ei] = self._ros_node.get_clock().now()

                        # optional: immediately write to sim if you want ROS to override
                        if self.use_ros_cmd:
                            self.robots[rk].write_joint_position_to_sim(self._ros_desired_q[rk][ei], env_ids=torch.tensor([ei], device=self.device))
                    return _cb

                self._ros_cmd_sub[robot_key][env_i] = self._ros_node.create_subscription(JointState, cmd_topic, _make_cb(robot_key, env_i), 10)
    ...
    def _js_msg(self, names, positions):
        msg = JointState()
        msg.header = Header()
    ...
    def _publish_robot_state(self, robot_key: str):
        ...
        q_all = robot.data.joint_pos.detach().cpu().numpy()
        dof = q_all.shape[1]
        for env_i in range(self.num_envs):
            ...
            pub.publish(self._js_msg(names, q))
            self._ros_spin_once()
