...
class FrankaDMPClothPlaceEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: FrankaDMPClothPlaceEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Names and desired commands per robot/env
        self._ros_joint_names = {}                  # robot_key -> [names]
        self._ros_desired_q   = {}                  # robot_key -> (num_envs, dof) tensor
        self._ros_desired_t   = {}                  # robot_key -> [rclpy.time.Time or None] per env
        self.use_ros_cmd      = True                # obey incoming cmds (can toggle at runtime)
        ...
        # Nested dicts: robot_key -> env_i -> publisher/subscriber
        self._ros_state_pub = {}
        self._ros_cmd_pub   = {}
        self._ros_cmd_sub   = {}

        for robot_key, robot in self.robots.items():
            # Resolve names (fall back if none available)
            names = getattr(robot.data, "joint_names", None)
            names = [str(n) for n in names] if names and len(names) else _default_franka
            self._ros_joint_names[robot_key] = names

            dof = robot.num_joints
            self._ros_desired_q[robot_key] = torch.zeros(self.num_envs, dof, device=self.device)
            self._ros_desired_t[robot_key] = [None for _ in range(self.num_envs)]

            # Per-robot containers
            self._ros_state_pub[robot_key] = {}
            self._ros_cmd_pub[robot_key]   = {}
            self._ros_cmd_sub[robot_key]   = {}

            # Choose a simple robot namespace: "robot1", "robot2", ...
            ns_robot = "robot1" if robot_key == "robot_1" else "robot2"

            # Build per-env topics and subs
            for env_i in range(self.num_envs):
                # /env_<i>/<robotN>/joint_states  &  /env_<i>/<robotN>/command_joint_state
                base_ns    = f"/env_{env_i}/{ns_robot}"
                state_topic = f"{base_ns}/joint_states"
                cmd_topic   = f"{base_ns}/joint_trajectory_command"

                # Pubs
                self._ros_state_pub[robot_key][env_i] = self._ros_node.create_publisher(JointState, state_topic, 10)
                # self._ros_cmd_pub[robot_key][env_i]   = self._ros_node.create_publisher(JointState, cmd_topic, 10)

                # Subs: capture robot_key/env_i in a closure
                def _make_cb(rk, ei):
                    def _cb(msg: JointTrajectory):
                        ...
                        # Choose the FIRST point in the trajectory (as requested)
                        pt = msg.points[0]
                        print('receiving jtraj command')

                        # If the trajectory has names, map by name; otherwise fallback by index
                        dof_local = self.robots[rk].num_joints
                        q_target = self.robots[rk].data.joint_pos[ei].clone()  # start from current sim joints (pad/fill)

                        if msg.joint_names and len(pt.positions) >= 1:
                            # name->index in incoming traj
                            name_to_idx = {n: i for i, n in enumerate(msg.joint_names)}
                            # map onto our robot joint order (truncate to dof_local)
                            for j, n in enumerate(self._ros_joint_names[rk][:dof_local]):
                                src = name_to_idx.get(n)
                                if src is not None and src < len(pt.positions):
                                    q_target[j] = float(pt.positions[src])
                        else:
                            # no names: use index alignment up to available positions
                            n_copy = min(dof_local, len(pt.positions))
                            if n_copy == 0:
                                return
                            q_target[:n_copy] = torch.tensor(pt.positions[:n_copy], device=self.device)

                        # record for bookkeeping
                        self._ros_desired_q[rk][ei] = q_target
                        self._ros_desired_t[rk][ei] = self._ros_node.get_clock().now()
                        print(q_target)

                        # write into sim immediately, if enabled
                        if self.use_ros_cmd:
                            self.robots[rk].write_joint_position_to_sim(
                                q_target, env_ids=torch.tensor([ei], device=self.device)
                            )
                            self.robots[rk].set_joint_position_target(
                                q_target, env_ids=torch.tensor([ei], device=self.device)
                            )
                    return _cb

                self._ros_cmd_sub[robot_key][env_i] = self._ros_node.create_subscription(
                    JointTrajectory, cmd_topic, _make_cb(robot_key, env_i), 10
                )

                # self.robot_1_rbs = franka_ros2(ns='env_0/robot1')
                # self.robot_2_rbs = franka_ros2(ns='env_0/robot2')
    ...
    def _publish_robot_state(self, robot_key: str):
        # robot.data.joint_pos: [num_envs, dof]
        robot = self.robots[robot_key]
        names = self._ros_joint_names[robot_key]
        ...
        for env_i in range(self.num_envs):
            ...
            pub = self._ros_state_pub[robot_key][env_i]
            pub.publish(self._js_msg(names, q, v))
            self._ros_spin_once()

    def _publish_robot_command(self, robot_key: str, q_target: torch.Tensor):
        """
        q_target: [num_envs, dof] (or broadcastable)
        Publishes to /env_<i>/<robotN>/command_joint_state for each env.
        """
        names = self._ros_joint_names[robot_key]
        qt = q_target.detach().cpu()
        if qt.ndim == 1:
            qt = qt.unsqueeze(0).repeat(self.num_envs, 1)

        for env_i in range(self.num_envs):
            q = qt[env_i].tolist()
            if len(q) < len(names): q += [0.0] * (len(names) - len(q))
            if len(q) > len(names): q = q[:len(names)]
            self._ros_cmd_pub[robot_key][env_i].publish(self._js_msg(names, q))
            self._ros_spin_once()
    ...
    def _apply_action(self):
        ...
        #     self.handle_1_init = self._handle_1.data.root_state_w.clone()
        #     self.handle_2_init = self._handle_2.data.root_state_w.clone()
        # cloth_positions = self._cloth_plain.root_physx_view.get_positions().clone()
        # cloth_positions[:,-3:] = self.cloth_corner_init
        # self._cloth_plain.root_physx_view.set_positions(cloth_positions, torch.arange(0, self.num_envs, device=self.device))
        # self._handle_1.write_root_state_to_sim(self.handle_1_init, torch.arange(0, self.num_envs, device=self.device))
        # self._handle_2.write_root_state_to_sim(self.handle_2_init, torch.arange(0, self.num_envs, device=self.device))
        ...
        self.inverse_kinematics_rbs(y)
        for robot_key in self.robots.keys():
            # q_des = self.robot_dof_targets[robot_key][:, :7]
            # q_smooth = self.lp1[robot_key].step(q_des)
            # self.robot_dof_targets[robot_key][:, :7] = q_smooth

            # NEW: publish all envs' states and commands for this robot
            ...
            self.robots[robot_key].set_joint_position_target(
                self.robot_dof_targets[robot_key])
            self._publish_robot_state(robot_key)
            # self._publish_robot_command(robot_key, self.robot_dof_targets[robot_key])
    ...
    def inverse_kinematics_rbs(self, y):
        current_absolute_pose = self._get_absolute_pose()
        ...
        try:
            current_joint_pos = self._robot_1.data.joint_pos[:, self.robot_entity_cfg['robot_1'].joint_ids]

            target_q_r = self.robot_1_rbs.IKin(rarm_pose_b.squeeze().cpu(), current_joint_pos.squeeze().cpu())[0]
            # target_q_r = self.robot_1_rbs.IKin_analytical(x2t(rarm_pose_b.squeeze().cpu()), 0.5, 
            #                                               current_joint_pos.squeeze().cpu().numpy())[0]

            current_joint_pos = self._robot_2.data.joint_pos[:, self.robot_entity_cfg['robot_2'].joint_ids]

            target_q_l = self.robot_2_rbs.IKin(larm_pose_b.squeeze().cpu(), current_joint_pos.squeeze().cpu())[0]
            # target_q_l = self.robot_2_rbs.IKin_analytical(x2t(larm_pose_b.squeeze().cpu()), 0.5, 
            #                                               current_joint_pos.squeeze().cpu().numpy())[0]

            if torch.isnan(torch.tensor(target_q_l, device=self.device)).any() or torch.isnan(torch.tensor(target_q_r, device=self.device)).any():
                raise ValueError
            else:
                self.robot_dof_targets['robot_1'][:, 0:7] = torch.tensor(target_q_r, device=self.device)
                self.robot_dof_targets['robot_2'][:, 0:7] = torch.tensor(target_q_l, device=self.device)
