...
class FrankaDMPClothPlaceEnv(DirectRLEnv):
    ...
    def create_cloth(self, stage, env_idx):
        ...
        cloth_prim.GetAttribute('physxParticle:particleEnabled').Set(True)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        ### Set cloth node goal
        dmp_parameters[:, 3] = dmp_parameters[:, 3].clamp(-0, clamp_loc)
        dmp_parameters[:, 4] = dmp_parameters[:, 4].clamp(-0, clamp_loc)
        dmp_parameters[:, 5] = dmp_parameters[:, 5].clamp(0.1, clamp_loc)
        dmp_parameters[:,6:-1] = dmp_parameters[:,6:-1] * clamp_loc / 1.6
        ...
        #     dmp_parameters[i, 0:3] = torch.tensor(self.simcloth.positions[i*self.n**2], device=self.device)
    ...
    def _apply_action(self):
        ...
        y[:,2] = y[:,2].clamp(0.1, clamp_loc)
        y += self.scene.env_origins

        if not hasattr(self, 'cloth_corner_init'):
            self.cloth_corner_init = self._get_cloth_positions_flat()[:,0:3].clone()
        for i in range(self.num_envs):
            cloth_positions = self._get_cloth_positions_flat()[i]
            cloth_positions[0:3] = self.cloth_corner_init[i]
            self._cloth_objects[i].root_physx_view.set_positions(cloth_positions, torch.tensor([0], device=self.device))
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        if not hasattr(self, "default_states") or self.default_states is None:
            self.default_states = {}
            for i in range(self.num_envs):
                self.default_states["_cloth_plain_pos" + str(i)] = self._cloth_objects[i].root_physx_view.get_positions()
                self.default_states["_cloth_plain_vel" + str(i)] = self._cloth_objects[i].root_physx_view.get_velocities()

        for i in range(self.num_envs):
            self._cloth_objects[i].root_physx_view.set_positions(
                self.default_states["_cloth_plain_pos" + str(i)][0], indices=torch.tensor([0], device=self.device))
            self._cloth_objects[i].root_physx_view.set_velocities(
                self.default_states["_cloth_plain_vel" + str(i)][0], indices=torch.tensor([0], device=self.device))
